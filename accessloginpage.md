...
2. **Teachers**: your full name (exact capitalization) + password.
...
All data lives in Supabase — the browser only remembers who is currently logged in on this device.
...
| | |
|---|---|
| **Login** | The teacher's **full name** (exact capitalization) |
| **Password** | Set by the admin — default is `tch123` |

Important: no teachers exist until you create one. To create a teacher:

1. Log in as admin (above).
2. Go to **Teachers** → **Add Teacher**.
3. Fill the form. The Password field comes prefilled with `tch123` — keep it or type your own.
4. Log in at `/login` using that full name + password.

Example: if you added "Sami Bouzid" with password `tch123`, you log in with exactly `Sami Bouzid` — the name must match letter-for-letter including capitalization, and the password must be exact.

Note: teacher names are matched case-**sensitively** — log in with the exact capitalization used when the teacher was created.
...
| Teacher | Full name (exact case) | `tch123` |
